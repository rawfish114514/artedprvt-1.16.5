...
public class MapGraphics {
    ...
    public MapGraphics(MapOperator operator){
        ...
        block= Blocks.WHITE_WOOL.defaultBlockState();
        drawCount=0;
    }
     ...
     */
    @ProgramUsable
    public void setBlock(BlockState blockIn){
        block=blockIn;
    }
    ...
    public void setBlock(Block blockIn){
        block=blockIn.defaultBlockState();
    }
    ...
    public void update(BlockPos pos){
        BlockPos thePos=pos.offset(center);
        Chunk chunk = world.getChunkAt(thePos);
        world.markAndNotifyBlock(thePos, chunk, null,null,2,0);
    }
    ...
    public void drawBlock(BlockPos pos){
        BlockPos thePos=pos.offset(center);
        Chunk chunk = world.getChunkAt(thePos);
        chunk.setBlockState(thePos, block,true);
        drawCount++;
    }
    ...
    public void drawCube(BlockPos pos1,BlockPos pos2){
        ...
        for(int cx=startChunk[0];cx<endChunk[0]+1;cx++){
            for(int cz=startChunk[1];cz<endChunk[1]+1;cz++){
                ...
                int hx=chunk.getPos().x<<4,hz=chunk.getPos().z<<4;
                ...
                for(int i=is;i<ie;i++){
                    for(int j=js;j<je;j++){
                        for(int k=ks;k<ke;k++){
                            chunk.setBlockState(new BlockPos(i,j,k),block,true);
                        }
                    }
                }
            }
    ...
    public void drawColumn(BlockPos pos,int height){
        BlockPos startPos=new BlockPos(pos.getX(),Math.min(pos.getY(),pos.getY()+height),pos.getZ()).offset(center);
        int end=Math.max(pos.getY(),pos.getY()+height)+center.getY();
        Chunk chunk=world.getChunk(startPos.getX()>>4,startPos.getZ()>>4);
        for(int i=startPos.getY();i<end;i++){
            chunk.setBlockState(new BlockPos(startPos.getX(),i,startPos.getZ()),block,true);
        }
